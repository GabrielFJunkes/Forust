...
fn render_comments (comments: Vec<Comment>, post_id: i64, answers: &HashMap<i64, Comment>) -> Markup {
    html!(
        @for comment in comments{
            (render_comment(comment, post_id, answers))
        }
    )
}

fn render_comment (comment: Comment, post_id: i64, answers: &HashMap<i64, Comment>) -> Markup {
        ...
        div class=(format!("ml-2 mb-4 w-full border-l-4 border-{}-{}00 pl-2", COLORS.get(random_index).unwrap_or(&"indigo"), random_color)) {
            ...
            @for answer_id in comment.answers_id {
                @if answers.contains_key(&answer_id) {
                    (render_comment(answers.get(&answer_id).unwrap().clone(), post_id, answers))
                }
            }
        }
    )
            ...
            div class="w-4/5 lg:w-8/12" {
                ...
                div class="mt-6 px-5 py-6 bg-white rounded-lg shadow-md container flex justify-between" {
                    div class="w-full" {
                        ...
                        (render_comments(post.comments, post.id, &post.answers))
                    }
                }
            }
        }
