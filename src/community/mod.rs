...
use self::{structs::{Community, CommunityData, Tag}, api::get_community_data};

#[derive(sqlx::FromRow)]
struct Post { id: i64, titulo: String, body: String, user_id: i64, comunidade_id: i64, created_at: OffsetDateTime }

async fn render_posts_preview(state: &AppState, id: i64) -> Markup {
    let posts = get_post_data(&state.db, id).await;
...
async fn content(state: &AppState, community: Community, logged_in: bool) -> Markup {
    html!(
